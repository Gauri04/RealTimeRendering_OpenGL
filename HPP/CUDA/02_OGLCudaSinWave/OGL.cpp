...
int i, j, k;
cudaError_t grgCudaResult;
struct cudaGraphicsResource* grgCudaGraphicsResource = NULL;
GLuint grgVbo_gpu_position;
bool grgbOnGpu = false;
...
void Initialize()
{
	...
	//code
	// check cuda devices for interop
	int grdevCount;
	grgCudaResult = cudaGetDeviceCount(&grdevCount);
	if (grgCudaResult != cudaSuccess)
	{
		fprintf(grgpFile, "\n Error while getting cuda device count. Exiting now..");
		Uninitialize();
	}
	else if (grdevCount == 0)
	{
		fprintf(grgpFile, "\n 0 devices got for cuda");
		Uninitialize();
	}
	else
	{
	...
	glGenBuffers(1, &grgVbo_gpu_position);
	glBindBuffer(GL_ARRAY_BUFFER, grgVbo_gpu_position);
	glBufferData(GL_ARRAY_BUFFER, (sizeof(float))* GR_ARRAY_SIZE, NULL, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	// opengl - cuda interop buffer register
	grgCudaResult = cudaGraphicsGLRegisterBuffer(&grgCudaGraphicsResource, grgVbo_gpu_position, cudaGraphicsMapFlagsWriteDiscard);
	if (grgCudaResult != cudaSuccess)
	{
		fprintf(grgpFile, "\n error while converting vbo gpu to cuda's graphic resource");
		Uninitialize();
	}
...
void Display(void)
{
	...
	if (grgbOnGpu == true)
	{
		...
		grgCudaResult = cudaGraphicsMapResources(1, &grgCudaGraphicsResource, 0);
		if (grgCudaResult != cudaSuccess)
		{
			fprintf(grgpFile, "\n error while mapping graphics resource to cuda, exiting now..");
			Uninitialize();
		}
		// get pointer from cuda
		float4* grpPos = NULL;
		size_t grnumBytes;
		grgCudaResult = cudaGraphicsResourceGetMappedPointer((void **)&grpPos, &grnumBytes, grgCudaGraphicsResource);
		if (grgCudaResult != cudaSuccess)
		{
			fprintf(grgpFile, "\n Error while grtting pointer from cuda. exiting now..");
			Uninitialize();

		}
		// pass pointer to cuda kernel
		launchCudaKernel(grpPos, MESH_WIDTH, MESH_HEIGHT, grgAnimationTime);
		// unmap pointer and bind to vbo_gpu
		cudaGraphicsUnmapResources(1, &grgCudaGraphicsResource, 0);
		glBindBuffer(GL_ARRAY_BUFFER, grgVbo_gpu_position);
...
void Uninitialize(void)
{
	...
	if (grgCudaGraphicsResource)
	{
		cudaGraphicsUnregisterResource(grgCudaGraphicsResource);
		grgCudaGraphicsResource = NULL;
	}
	...
	if (grgVbo_gpu_position)
	{
		glDeleteBuffers(1, &grgVbo_gpu_position);
		grgVbo_position = 0;
	}
