package com.example.graphtriangle;
	...
	OnGestureListener, OnDoubleTapListener
{
	...
	private int[] grvboTriangle = new int[1];
	...
	private void initialize(GL10 gl)
	{
		...
		float[] grtriangleVertices = new float[]
		{
				0.0f, 0.5f, 0.0f,
				-0.5f, -0.5f, 0.0f,
				0.5f, -0.5f, 0.0f
		...
		// bytebuffer for triangle vertices
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(grtriangleVertices.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer verticesBuffer = byteBuffer.asFloatBuffer();
		verticesBuffer.put(grtriangleVertices);
		...
		GLES32.glGenBuffers(1, grvboTriangle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, grvboTriangle[0]);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, grtriangleVertices.length * 4, verticesBuffer, GLES32.GL_STATIC_DRAW);
		...
		byteBuffer = ByteBuffer.allocateDirect(grverticalLineVertices1.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
	...
	private void display()
	{
		...
		// bind vao
		GLES32.glBindVertexArray(grvaoTriangle[0]);
		GLES32.glVertexAttrib3f(GLESMacros.GR_ATTRIBUTE_COLOR, 1.0f, 1.0f, 0.0f);
	...
	void uninitialize()
	{
		...
		if(grvboTriangle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, grvboTriangle, 0);
			grvboTriangle[0] = 0;
		}
