	...
	OnGestureListener, OnDoubleTapListener
{
	...
	private  int[] grvboColor = new int[1];
	...
	float grhorizontalLineVertices[] = new float[]{-3.0f, 0.0f, 3.0f, 0.0f, -3.0f, 0.5f, 3.0f, 0.5f};
	...
	private void initialize(GL10 gl)
	{
		...
		float[] grcolor = new float[80 * 3];
		for(index = 0; index < 240; index = index + 3)
		{
			grcolor[index] = 0.0f;
			grcolor[index] = 1.0f;
			grcolor[index] = 0.0f;
		...
		/*
		// color
		GLES32.glGenBuffers(1, grvboColor, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, grvboColor[0]);
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(grcolor.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer colorBuffer = byteBuffer.asFloatBuffer();
		colorBuffer.put(grcolor);
		colorBuffer.position(0);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, grcolor.length * 4, colorBuffer, GLES32.GL_STATIC_DRAW);
		...
		*/
		...
		// bytebuffer for horizontal line vertices - top

		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(grhorizontalLineVertices1.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		...
		// bytebuffer for horizontal line vertices - bottom
	...
	private void display()
	{
		...
		Matrix.translateM(grtranslateMatrix, 0, 0.0f, 0.0f, -3.0f);
		...
		// ********* Horizontal Line vertices - bottom ***********************88
		Matrix.setIdentityM(grmodelViewMatrix, 0);
		Matrix.setIdentityM(grtranslateMatrix, 0);
		Matrix.setIdentityM(grmodelViewProjectionMatrix, 0);
