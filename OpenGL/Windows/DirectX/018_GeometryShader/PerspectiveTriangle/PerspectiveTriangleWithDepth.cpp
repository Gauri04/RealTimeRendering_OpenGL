...
HRESULT Initialize()
{
	...
	const char* vertexShadereSourceCode =
		...
		"return(output);" \
		"}";
	// there is no SV_POSITION in vertex shader, this means that vertex shader will not give position
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n D3DCompile() failed for geometry shader : %s", (char*)pID3DBlob_Error->GetBufferPointer());
			fclose(grgpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n COM error while compiling geometry shader");
			fclose(grgpFile);
		}
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n D3DCompile() succeeded for geometry shader");
		fclose(grgpFile);
	}
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "ID3D11Device::CreateGeometryShader() failed");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateGeometryShader() succeeded");
		fclose(grgpFile);
	}
	...
	d3d11InputElementDesc.SemanticName = "POSITION";					// this is the 'POSITION' which is there in vertex shader in main - main(float4 pos : POSITION)
	...
	d3d11InputElementDesc.Format = DXGI_FORMAT_R32G32B32_FLOAT;			// format of data to be passed - 3 vertices (x,y,z is rgb) and type (float)
	...
	float verticesSquare[] =												// It follows CW winding order (ClockWise) not CCW like OGL or directx is left handed while ogl is right handed
	{
		0.0f, 1.0f, 0.0f,
		1.0f, -1.0f, 0.0f,
		-1.0f, -1.0f, 0.0f
	...
	// set constant buffer for geometry shader
