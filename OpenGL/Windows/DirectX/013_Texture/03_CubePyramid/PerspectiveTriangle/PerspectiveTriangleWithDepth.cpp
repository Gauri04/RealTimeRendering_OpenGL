...
#include"WICTextureLoader.h"				// WIC = Windows Imaging Component, library for Direct2D based textures
//using namespace DirectX;					// this is namespace for DirectX lib for texture but as we will call only one function, we havent included it
...
ID3D11SamplerState* gpID3D11SamplerState_SamplerTexture = NULL;
...
HRESULT Initialize()
{
	...
	float pyramidTexCoord[] =	// r-g-b color
	{
		+0.5, +1.0,
		+1.0, +0.0,
		+0.0, +0.0,
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidVertices);
	...
	// here we havent set this buffer in pipeline because as its dynamic draw, we will set it in display()
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;								// DirectX favors dynamic draw unlike static draw in OGL because DirectX favors multithreading
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidTexCoord);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for vertex buffer - texcoord");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for vertex buffer - texcoord");
		fclose(grgpFile);
	}
	// Push the data in this buffer by  Map and Unmap
	// copy vertices into above vertex buffer
	...
	// texture
	//hr = LoadD3DTexture(MAKEINTRESOURCE(GRTEXTURE_STONE), &gpID3D11ShaderResourceView_Texture);
	...
	//hr = LoadD3DTexture(MAKEINTRESOURCE(GRTEXTURE_KUNDALI), &gpID3D11ShaderResourceView_TextureRectangle);
	hr = LoadD3DTexture(L"Kundali.bmp", &gpID3D11ShaderResourceView_TextureRectangle);
	if (FAILED(hr))
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n error in LoadD3DTexture");
	...
	hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpID3D11SamplerState_SamplerTexture);
	if (FAILED(hr))
	{
		fopen_s(&grgpFile, logFileName, "a+");
...
HRESULT LoadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	//{
		...
		//result = GetBitmapBits(hBitmap, bmp.bmHeight * bmp.bmWidth, lpBitmapData);
		//hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, gpID3D11DeviceContext, lpBitmapData, sizeof(lpBitmapData),
		//	&textureResource, ppID3D11ShaderResourceView, sizeof(lpBitmapData));
		//DirectX::CreateWICTextureFromMemory(gpID3D11Device, NULL, 0,  &textureResource, ppID3D11ShaderResourceView, 0);
...
void Display(void)
{
	...
	offset = 0;
	// set vertex buffer into pipeline - texture
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_TexCoord, &stride, &offset);
	// params of abov function : 
		// 1st param - index which is 0 for vertex
		// 2nd - how many buffers
		// which buffer - 
		// stride 
		// offset
	...
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_SamplerTexture);

	// draw
	gpID3D11DeviceContext->Draw(12, 0);
	// params of above function
		//1st - how many vertices
		// 2nd  - starting 
...
{
	// code
	if (gpID3D11SamplerState_SamplerTexture)
	{
		gpID3D11SamplerState_SamplerTexture->Release();
		gpID3D11SamplerState_SamplerTexture = NULL;
	}
