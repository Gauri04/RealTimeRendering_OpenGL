...
HRESULT Initialize()
{
	...
	HRESULT Resize(int, int);
	void Uninitialize(void);
	HRESULT LoadD3DTexture(const LPCTSTR, ID3D11ShaderResourceView**);
...
HRESULT LoadD3DTexture(const LPCTSTR resourceID, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	// variables
	HRESULT hr;
	HBITMAP hBitmap;
	...
	if (hBitmap)
	{
		...
		hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, (const uint8_t*)bmp.bmBits, bmp.bmWidth * bmp.bmHeight,
			&textureResource, ppID3D11ShaderResourceView, 0);
