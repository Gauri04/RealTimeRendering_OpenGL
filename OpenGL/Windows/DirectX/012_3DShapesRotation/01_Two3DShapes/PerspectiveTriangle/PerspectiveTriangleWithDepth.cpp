...
HRESULT Initialize()
{
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for vertex buffer - color pyramid");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for vertex buffer - color pyramid");
		fclose(grgpFile);
	}
	// Push the data in this buffer by  Map and Unmap
	// copy vertices into above vertex buffer
