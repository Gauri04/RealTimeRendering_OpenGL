...
HRESULT Initialize()
{
	...
	const char* vertexShadereSourceCode =
		...
		"for(i = 0; i < 3; i++)" \
		"{" \
		"light_direction[i] = (float3)normalize(lightPosition[i] - eye_position);" \
		...
		"specular[i] = ((float3)ls[i]) * ks * pow(max(dot(reflection_vector[i], viewer_vector), 0.0), materialShinyness);" \
		"output.phong_ads_light = output.phong_ads_light + ambient[i] + diffuse[i] + specular[i];" \
		"}" \
		"}" \
		"else" \
		"{" \
	...
	const char* vertexShadereSourceCodePP =
		...
		"{" \
			...
			"float4 eye_position = mul(worldMatrix, pos);" \
			"eye_position = mul(viewMatrix, eye_position);" \
			"output.view_vector = -eye_position.xyz;" \
			"output.transformed_normals = (float3)mul((float3x3)worldMatrix, (float3)normals);" \
			"output.light_direction1 = ((float3)lightPosition[0] - eye_position.xyz);" \
			"output.light_direction2 = ((float3)lightPosition[1] - eye_position.xyz);" \
			"output.light_direction3 = ((float3)lightPosition[2] - eye_position.xyz);" \
		"}" \
		"float4 position = mul(worldMatrix, pos);" \
		"position = mul(viewMatrix, position);" \
		"position = mul(projectionMatrix, position);"
	...
	const char* pixelShaderSourceCodePP =
		...
		"{" \
			"int i;" \
			"phong_ads_light = float3(0.0, 0.0, 0.0);" \
			"float3 reflection_vector[3];" \
			"float3 ambient[3];" \
			"float3 diffuse[3];" \
			"float3 specular[3];" \
			...
			"{" \
				...
				"specular[i] = ((float3)ls[i]) * ks * pow(max(dot(reflection_vector[i], normalized_view_vector), 0.0), materialShinyness);" \
	...
	grLight[0].lightAmbientColor[0] = 0.0f;
	grLight[0].lightAmbientColor[1] = 0.0f;
	grLight[0].lightAmbientColor[2] = 0.0f;
	grLight[0].lightAmbientColor[3] = 1.0f;

	grLight[0].lightDiffuseColor[0] = 1.0f;
	grLight[0].lightDiffuseColor[1] = 0.0f;
	grLight[0].lightDiffuseColor[2] = 0.0f;
	grLight[0].lightDiffuseColor[3] = 1.0f;

	grLight[0].lightSpecularColor[0] = 1.0f;
	grLight[0].lightSpecularColor[1] = 0.0f;
	grLight[0].lightSpecularColor[2] = 0.0f;
	grLight[0].lightSpecularColor[3] = 1.0f;

	grLight[0].lightPosition[0] = 0.0f;
	grLight[0].lightPosition[1] = 0.0f;
	grLight[0].lightPosition[2] = 0.0f;
	grLight[0].lightPosition[3] = 1.0f;

	// 2nd Light
	grLight[1].lightAmbientColor[0] = 0.0f;
	grLight[1].lightAmbientColor[1] = 0.0f;
	grLight[1].lightAmbientColor[2] = 0.0f;
	grLight[1].lightAmbientColor[3] = 1.0f;

	grLight[1].lightDiffuseColor[0] = 0.0f;
	grLight[1].lightDiffuseColor[1] = 1.0f;
	grLight[1].lightDiffuseColor[2] = 0.0f;
	grLight[1].lightDiffuseColor[3] = 1.0f;

	grLight[1].lightSpecularColor[0] = 0.0f;
	grLight[1].lightSpecularColor[1] = 1.0f;
	grLight[1].lightSpecularColor[2] = 0.0f;
	grLight[1].lightSpecularColor[3] = 1.0f;

	grLight[1].lightPosition[0] = 0.0f;
	grLight[1].lightPosition[1] = 0.0f;
	grLight[1].lightPosition[2] = 0.0f;
	grLight[1].lightPosition[3] = 1.0f;

	// 3rd Light
	grLight[2].lightAmbientColor[0] = 0.0f;
	grLight[2].lightAmbientColor[1] = 0.0f;
	grLight[2].lightAmbientColor[2] = 0.0f;
	grLight[2].lightAmbientColor[3] = 1.0f;

	grLight[2].lightDiffuseColor[0] = 0.0f;
	grLight[2].lightDiffuseColor[1] = 0.0f;
	grLight[2].lightDiffuseColor[2] = 1.0f;
	grLight[2].lightDiffuseColor[3] = 1.0f;

	grLight[2].lightSpecularColor[0] = 0.0f;
	grLight[2].lightSpecularColor[1] = 0.0f;
	grLight[2].lightSpecularColor[2] = 1.0f;
	grLight[2].lightSpecularColor[3] = 1.0f;

	grLight[2].lightPosition[0] = 0.0f;
	grLight[2].lightPosition[1] = 0.0f;
