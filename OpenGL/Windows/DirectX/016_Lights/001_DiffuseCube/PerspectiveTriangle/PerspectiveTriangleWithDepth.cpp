...
HRESULT Initialize()
{
	...
	const char* vertexShadereSourceCode =
		...
		"if(keyPressed == 1)" \
		"{" \
			"float eye_position = mul(worldViewMatrix, pos);" \
			"float3 t_norm = normalize(mul((float3x3)worldViewMatrix, (float3)normal));" \
			"float3 s = (float3)normalize(lightPosition - eye_position);" \
			"output.diffuse_light = ld * kd * max(dot(s, t_norm), 0.0);" \
		"}" \
		"else" \
		"{" \
...
void Display(void)
{
	...
	if (gbLight == true)
	{
		constantBuffer.KeyPressed = true;
