...
ID3D11HullShader* gpID3D11HullShader = NULL;			// same as Tessellation Control shader from OGL
ID3D11DomainShader* gpID3D11DomainShader = NULL;		// same as Tessellation Evaluation shader from OGL
...
{
	XMVECTOR Hull_Constant_Functions_Params;			// XMVECTOR is array of 4 floats, whose 0th member will be used as no. of line strips from OGL 
														// And 1st member will be used as no. of line segments as from OpenGL
};
...
HRESULT Initialize()
{
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n D3DCompile() failed for hull shader : %s", (char*)pID3DBlob_Error->GetBufferPointer());
			fclose(grgpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n COM error while compiling hull shader");
			fclose(grgpFile);
		}
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n D3DCompile() succeeded for hull shader");
		fclose(grgpFile);
	}
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "ID3D11Device::CreateHullShader() failed");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateHullShader() succeeded");
		fclose(grgpFile);
	}
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n D3DCompile() failed for domain shader : %s", (char*)pID3DBlob_Error->GetBufferPointer());
			fclose(grgpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
		...
		{
			fopen_s(&grgpFile, logFileName, "a+");
			fprintf(grgpFile, "\n COM error while compiling domain shader");
			fclose(grgpFile);
		}
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n D3DCompile() succeeded for domain shader");
		fclose(grgpFile);
	}
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "ID3D11Device::CreateDomainShader() failed");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateDomainShader() succeeded");
		fclose(grgpFile);
	}

	// set domain shader for pipeline
	...
	d3d11InputElementDesc.SemanticName = "POSITION";					// this is the 'POSITION' which is there in vertex shader in main - main(float2 pos : POSITION)
	d3d11InputElementDesc.SemanticIndex = 0;							// if we are going to send multiple geometries in same semantics, then they are separted by indices
	d3d11InputElementDesc.Format = DXGI_FORMAT_R32G32_FLOAT;			// format of data to be passed - 2 vertices (x,y is rg of rgb) and type (float)
	d3d11InputElementDesc.InputSlot = 0;								// we have given slot 0 for position, can give slot 1 for color etc. line enums in OGL (GR_ATTRIB_POSITION)
	d3d11InputElementDesc.AlignedByteOffset = 0;						// how many gap is there in case of multiple geometries in senamtics
	d3d11InputElementDesc.InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;	// we will draw vertex, not instance so PER_VERTEX_DATA 
	d3d11InputElementDesc.InstanceDataStepRate = 0;						// if data is per instance, then how many gap is there
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for vertex buffer - position vertices");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for vertex buffer - position vertices");
		fclose(grgpFile);
	}
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for constant buffer - hull shader");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for constant buffer - hull shader");
		fclose(grgpFile);
	}

	// set this constant buffer (for uniform) in pipeline
	gpID3D11DeviceContext->HSSetConstantBuffers(0,				// position is set to 0th index hence we have passed the index here
		1,														// how many buffers
		&gpID3D11Buffer_ConstantBuffer_HullShader);

	// *************************** ocnstant buffer - domain shader
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for constant buffer - domain shader");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for constant buffer - domain shader");
		fclose(grgpFile);
	}

	// set this constant buffer (for uniform) in pipeline
	gpID3D11DeviceContext->DSSetConstantBuffers(0,				// position is set to 0th index hence we have passed the index here
		1,														// how many buffers
	...
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() failed for constant buffer - pixel shader");
		fclose(grgpFile);
		return(hr);
	}
	else
	{
		fopen_s(&grgpFile, logFileName, "a+");
		fprintf(grgpFile, "\n ID3D11Device::CreateBuffer() succeeded for constant buffer - pixel shader");
		fclose(grgpFile);
	}

	// set this constant buffer (for uniform) in pipeline
	gpID3D11DeviceContext->PSSetConstantBuffers(0,				// position is set to 0th index hence we have passed the index here
		1,														// how many buffers
...
void Display(void)
{
	...
	// ************************* display line *************************/
	UINT stride = sizeof(float) * 2; // jump
	UINT offset = 0;
	...
	ZeroMemory(&constantBuffer_DomainShader, sizeof(constantBuffer_DomainShader));
	...
	// chbuffer for hull shader
	CBUFFER_HullShader constantBuffer_HullShader;
	ZeroMemory(&constantBuffer_HullShader, sizeof(constantBuffer_HullShader));
	// below, 1st param is number of line strips,
	// 2nd param - no. of line segments
	constantBuffer_HullShader.Hull_Constant_Functions_Params = XMVectorSet(1.0f, (FLOAT)guiNumberOfLineSegments, 0.0f, 1.0f);
	...
	ZeroMemory(&constantBuffer_PixelShader, sizeof(constantBuffer_PixelShader));
