...
void Initialize()
{
	...
	const GLchar* grgeometryShaderSourceCode =					// also called as "pass-through shader" as it does not have any code (main is empty, no code is there in main)
		"#version 450 core" \
		"\n" \
		"layout(triangles)in;" \
...
void Uninitialize(void)
{
	...
	glDetachShader(grgShaderProgramObject, grgGeometryShaderObject);
	// detach fragment shader
	glDetachShader(grgShaderProgramObject, grgFragmentShadeerObject);

	// delete vertex object
	glDeleteShader(grgGeometryShaderObject);
	grgGeometryShaderObject = 0;
