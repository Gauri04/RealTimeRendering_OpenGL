...
-(void)initialize
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 2 * sizeof(int), NULL, GL_DYNAMIC_DRAW);
...
-(void)drawView:(id)sender
{
    ...
    GLfloat grVerticalLineVertices[] =
    {
        0.0f, -3.0f,
        0.0f, 3.0f
    };
    GLfloat grHotizontalLineVertices[] =
    {
        -6.0f, 0.0f,
        6.0f, 0.0f
    };
    ...
    glBindVertexArray(grgVao);
    glBindBuffer(GL_ARRAY_BUFFER, grgVbo_position);
    glBufferData(GL_ARRAY_BUFFER, 2 * 2 * sizeof(GL_FLOAT), grVerticalLineVertices, GL_DYNAMIC_DRAW);
    glVertexAttribPointer(GR_ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(GR_ATTRIBUTE_POSITION);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    glBindVertexArray(grgVao);
    glBindBuffer(GL_ARRAY_BUFFER, grgVbo_position);
    glBufferData(GL_ARRAY_BUFFER, 2 * 2 * sizeof(GL_FLOAT), grHotizontalLineVertices, GL_DYNAMIC_DRAW);
    glVertexAttribPointer(GR_ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(GR_ATTRIBUTE_POSITION);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glVertexAttrib3f(GR_ATTRIBUTE_COLOR, 1.0f, 0.0f, 0.0f);
