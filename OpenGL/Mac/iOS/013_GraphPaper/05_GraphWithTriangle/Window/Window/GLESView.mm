...
-(void)initialize
{
    ...
    glGenVertexArrays(1, &grgVaoTriangle);
    glBindVertexArray(grgVaoTriangle);

    glGenBuffers(1, &grgVbo_position_triangle);
    glBindBuffer(GL_ARRAY_BUFFER, grgVbo_position_triangle);
    glBufferData(GL_ARRAY_BUFFER, sizeof(grtriangleVerts), grtriangleVerts, GL_STATIC_DRAW);
...
-(void)uninitialize
{
    ...
    if (grgVaoTriangle)
    {
        glDeleteVertexArrays(1, &grgVaoTriangle);
        grgVaoTriangle = 0;
    }
